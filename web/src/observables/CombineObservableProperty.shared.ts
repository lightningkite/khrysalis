...
export class CombineObservableProperty<T, A, B> extends ObservableProperty<T> {
    ...
    public get onChange(): Observable<T> { return rxCombineLatest([rxConcat(rxOf(this.observableA.value), this.observableA.onChange), rxConcat(rxOf(this.observableB.value), this.observableB.onChange)]).pipe(rxMap(x => ((a: A, b: B): (any | null) => this.combiner(a, b))(x[0], x[1]))).pipe(rxSkip(0)); }
...
export function comLightningkiteKhrysalisObservablesObservablePropertyCombine<T, B, C>(this_: ObservableProperty<T>, other: ObservableProperty<B>, combiner:  ((a: T, b: B) => C)): ObservableProperty<C> {
    return new CombineObservableProperty<(any | null), (any | null), (any | null)>(this_, other, combiner);
}
