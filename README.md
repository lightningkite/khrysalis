...
Dual licensed under GPLv3 and private commercial license - prices yet to be determined.  Code generated by Khrysalis is under the same license as was used to obtain Khrysalis.
...
Until the library is published in public repositories, you'll have to look at the [local installation section](#local-installation) to get Khrysalis installed on your machine locally.

### Example Project

You can see [the example project here.](https://github.com/lightningkite/khrysalis-template)  

### Local Installation

To install a local copy of Khrysalis, do the following:

- Clone the repository
- Run `./firstTime.sh` if on Mac or Linux, `./firstTime.bat` if on Windows
    - Note you will still only be able to compile for iOS on a Mac
...
// Configures the app to have tasks for Khrysalis.
project.configureGradle(
    // The package name of your app.
    packageName = packageName,
    // The folder your iOS project's contents are in.  Code and resources will be written into here.
    iosRelativeBase = "../../ios/MyProjectName",
    setupCodeConversion = {
        // When the code converts, these imports will be added to Swift convert files.
        // If you are using extensions to Khrysalis, you'll need to import their associated packages here.
        imports += listOf("KhrysalisMaps", "KhrysalisBluetooth")
    },
    // You can add additional instructions for the layout converter here - you'll likely never need to though.
    // Just keep this as is if you're using maps, or just don't specify this argument at al..
...
- `khrysalisIos`
    - Completely converts the project to iOS.  Performs all of the following tasks:
        - `khrysalisConvertKotlinToSwift`
            - Converts Kotlin to Swift for any files with `.shared.kt` at the end.
            - Creates stubs in Swift for any declarations in `.actual.kt`, sectioned via comments.
        - `khrysalisConvertLayoutsToSwift`
            - Converts the Android XML layouts into Swift classes, labeled Pascal-case appended with `XML`.
        - `khrysalisConvertResourcesToIos`
            - Converts various resources into something accessible in iOS
                - Drawables become Swift files that allow you to retrieve `CALayer`s
                - Strings are exposed in a single Swift file.
                - Dimensions are exposed in a single Swift file.
                - Colors are exposed in a single Swift file.
                - Color state lists are exposed in separate Swift files.
                - PNGs are placed into an `xcassets` folder.
- `khrysalisConvertKotlinToSwiftClean`
    - Converts Kotlin to Swift, but without any caching.
- `khrysalisCreateAndroidLayoutClasses`
    - Creates typed view reference files for the Android XML layouts, labeled Pascal-case appended with `XML`.  Kotlin equivalent to the Swift ones generated by `khrysalisConvertLayoutsToSwift`.
- `khrysalisPrototype`
    - Generates `ViewGenerator`s for each XML not starting with `component_` that navigate to the other views.  More information documented under [PrototypeGenerator.md](PrototypeGenerator.md)
- `khrysalisFlowDoc`
    - Part of the prototyping system - generates diagrams showing the flow between the screens of the app.
