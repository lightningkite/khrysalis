...
//            }
            ...
            ViewType("LinearLayout", "UIView", "View") { node ->
                ...
                val dividerText =
                    node.attributes["tools:iosDivider"] ?: "flex.addItem().height(1).backgroundColor(.gray)"
                ...
                node.children.forEachBetween(
                    forItem = { child ->
                        ...
                        val marginTop =
                            (child.attributeAsDimension("android:layout_marginTop") ?: defaultMargin).toString()
                        val marginLeft = (child.attributeAsDimension("android:layout_marginLeft")
                            ?: child.attributeAsDimension("android:layout_marginStart") ?: defaultMargin).toString()
                        val marginBottom =
                            (child.attributeAsDimension("android:layout_marginBottom") ?: defaultMargin).toString()
                        val marginRight = (child.attributeAsDimension("android:layout_marginRight")
                            ?: child.attributeAsDimension("android:layout_marginEnd") ?: defaultMargin).toString()
                        ...
                        child.attributeAsDimension("android:layout_width")?.takeUnless { it == "0" }?.let { s ->
                            append(".width($s)")
                        }
                        child.attributeAsDimension("android:layout_height")?.takeUnless { it == "0" }?.let { s ->
                            append(".height($s)")
                        }
                        child.attributeAsDimension("android:minWidth")?.let { s ->
                            append(".minWidth($s)")
            ...
            ViewType("FrameLayout", "UIView", "View") { node ->
                ...
                for (child in node.children) {
                    ...
                    when (child.attributes["android:layout_width"]) {
                        "match_parent" -> {
                            append(".left($marginLeft + $paddingLeft).right($marginRight + $paddingRight)")
                        }
                        "wrap_content", null -> {
                        }
                    ...
                    when (child.attributes["android:layout_height"]) {
                        "match_parent" -> {
                            append(".top($marginTop + $paddingTop).bottom($marginBottom + $paddingBottom)")
                        }
                        "wrap_content", null -> {
                        }
                    ...
                    val doHorizontalPlacement = child.attributes["android:layout_width"] != "match_parent"
                    val doVerticalPlacement = child.attributes["android:layout_height"] != "match_parent"
                    child.attributes["android:layout_gravity"]?.let {
                        for (part in it.split('|')) {
                            when (part) {
                                "left" -> if (doHorizontalPlacement) append(".left($marginLeft + $paddingLeft)")
                                "right" -> if (doHorizontalPlacement) append(".right($marginRight + $paddingRight)")
                                "start" -> if (doHorizontalPlacement) append(".start($marginLeft + $paddingLeft)")
                                "end" -> if (doHorizontalPlacement) append(".end($marginRight + $paddingRight)")
                                "center_horizontal" -> if (doHorizontalPlacement) append(".hCenter((-$marginLeft + $marginRight) / 2)")

                                "top" -> if (doVerticalPlacement) append(".top($marginTop + $paddingTop)")
                                "bottom" -> if (doVerticalPlacement) append(".bottom($marginBottom + $paddingBottom)")
                                "center_vertical" -> if (doVerticalPlacement) append(".vCenter((-$marginTop + $marginBottom) / 2)")

                                "center" -> {
                                    if (doHorizontalPlacement)
                                        append(".hCenter($marginLeft - $marginRight)")
                                    if (doVerticalPlacement)
