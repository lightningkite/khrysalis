...
fun convertLayoutsToHtml(
    ...
    stringsFile.bufferedWriter().use {
        with(SmartTabWriter(it)) {
            appendLine("import { DrawableResource } from 'butterfly-web/dist/views/DrawableResource'")
            appendLine("//! Declares $packageName.R")
            appendLine("export namespace R {")

            appendLine("export namespace color {")
            resources.colors.entries.sortedBy { it.key }.forEach {
                appendLine("export const ${it.key}: string = \"var(${it.value.cssName})\";")
            }
            appendLine("}")

            appendLine("export namespace drawable {")
            resources.drawables.entries.sortedBy { it.key }.forEach {
                val cssName = it.value.cssName
                val imagePath = it.value.imagePath
                if (imagePath != null) {
                    appendLine("export const ${it.key.safeJsIdentifier()}: DrawableResource = new DrawableResource(\"$cssName\", \"$imagePath\");")
                } else {
                    appendLine("export const ${it.key.safeJsIdentifier()}: DrawableResource = new DrawableResource(\"$cssName\");")
                }
            }
            appendLine("}")
            ...
            defaultStrings.keys.forEach {
                appendLine("${it.safeJsIdentifier()}: string;")
            }
            appendLine("}")
            ...
            defaultStrings.entries.forEach {
                appendLine(
                    "export const ${it.key.safeJsIdentifier()}: string = \"${
                        it.value.replace(
                            whitespace,
                            " "
                        )
            ...
            appendLine("export let _string = DefaultStrings;")
