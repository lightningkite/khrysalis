...
internal class ReflectionCache(reflectionCacheSize: Int) {
    ...
    private val javaClassToKotlin = HashMap<Class<Any>, KClass<Any>>(reflectionCacheSize)
    private val javaConstructorToKotlin = HashMap<Constructor<Any>, KFunction<Any>>(reflectionCacheSize)
    private val javaMethodToKotlin = HashMap<Method, KFunction<*>>(reflectionCacheSize)
    private val javaConstructorIsCreatorAnnotated = HashMap<AnnotatedConstructor, Boolean>(reflectionCacheSize)
    private val javaMemberIsRequired = HashMap<AnnotatedMember, BooleanTriState?>(reflectionCacheSize)

    fun kotlinFromJava(key: Class<Any>): KClass<Any> =
        javaClassToKotlin.get(key) ?: key.kotlin.let { javaClassToKotlin.getOrPut(key) { it } }

    fun kotlinFromJava(key: Constructor<Any>): KFunction<Any>? =
        javaConstructorToKotlin.get(key) ?: key.kotlinFunction?.let { javaConstructorToKotlin.getOrPut(key) { it } }

    fun kotlinFromJava(key: Method): KFunction<*>? =
        javaMethodToKotlin.get(key) ?: key.kotlinFunction?.let { javaMethodToKotlin.getOrPut(key) { it } }

    fun checkConstructorIsCreatorAnnotated(
        key: AnnotatedConstructor,
        calc: (AnnotatedConstructor) -> Boolean
    ): Boolean = javaConstructorIsCreatorAnnotated.get(key)
        ?: calc(key).let { javaConstructorIsCreatorAnnotated.getOrPut(key) { it } }

    fun javaMemberIsRequired(key: AnnotatedMember, calc: (AnnotatedMember) -> Boolean?): Boolean? =
        javaMemberIsRequired.get(key)?.value ?: calc(key).let {
            javaMemberIsRequired.getOrPut(key) { BooleanTriState.fromBoolean(it) }?.value ?: it
        }
}
            ...
            if (member.getParameterCount() > 0 && member.getDeclaringClass().isKotlinClass()) {
                return cache.checkConstructorIsCreatorAnnotated(member) {
                    ...
                    if (kConstructor != null) {
                        ...
                        val anyCompanionMethodIsJsonCreator =
                            member.type.rawClass.kotlin.companionObject?.declaredFunctions
                        ...
                        val areAllParametersValid =
                            kConstructor.parameters.size == kConstructor.parameters.count { it.name != null }
