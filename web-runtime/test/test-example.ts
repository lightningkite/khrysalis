...
class Superclass {
    equals(other: any): boolean {
        return other instanceof Superclass
    }
...
class Point<T> extends Superclass implements DataClass {
    ...
    public static propertiesJsonOverride = {x: "xx"}
...
describe("TestDataClass", () => {
    ...
    test("toJSON", () => {
        const point = new Point(32, 12)
        expect(JSON.stringify(point)).toEqual('{"xx":32,"y":12}')
    })
    test("fromJSON", () => {
        const parsed = JSON2.parse('{"xx":32,"y":12}', [Point])
        expect(parsed).toBeInstanceOf(Point)
        expect(parsed).toEqual(new Point(32, 12))
    })
})

function cycle<T>(value: T, type: ReifiedType<T>) {
    const asStr = JSON.stringify(value)
    console.log(asStr)
    const copy = JSON2.parse(asStr, type)
    console.log(copy)
    expect(JSON.stringify(copy)).toEqual(asStr)
}

describe("Serialization", () => {
    test("Set", () => {
        cycle(new Set([1, 2, 3, 4]), [Set, [Number]])
    })
    test("Map", () => {
        cycle(new Map([["asdf", 1], ["fdsa", 2]]), [Map, [String], [Number]])
    })
})